...
'''
 # 从容易开始学习.==============使用方法. aaa里面存开局空白棋盘, bbb存一个棋盘.来切割棋子. ccc存正则使用的棋盘
...
minshapes1=min([i[0] for i in shapes])
minshapes2=min([i[1] for i in shapes])
...
img_0 = cv2.imread('saolei_little_easy/0.png', 0)  # 写0表示以单通道进行读取图片!!!!!!!!!!
img_1 = cv2.imread('saolei_little_easy/1.png', 0)  # 写0表示以单通道进行读取图片!!!!!!!!!!
img_2 = cv2.imread('saolei_little_easy/2.png', 0)  # 写0表示以单通道进行读取图片!!!!!!!!!!
img_3 = cv2.imread('saolei_little_easy/3.png', 0)  # 写0表示以单通道进行读取图片!!!!!!!!!!
# img_4 = cv2.imread('saolei_little_easy/4.png', 0)  # 写0表示以单通道进行读取图片!!!!!!!!!!
img_100 = cv2.imread('saolei_little_easy/100.png', 0)  # 写0表示以单通道进行读取图片!!!!!!!!!!
...
for kk in zip(zuobielie, youbielie):
    for kk2 in zip(shagnhengxian, xiahengxian):
        tmplittle = qipan[kk2[0]:kk2[1], kk[0]:kk[1]]
        for dex,i in enumerate([img_0,img_1,img_2,img_3,img_100]):

            tmplittle=tmplittle[:minshapes1,:minshapes2]
            i=i[:minshapes1,:minshapes2]


            duoshao=tmplittle[1:-1,1:-1].size#公共多少像素.
            #==============需要一个鲁棒性更高的算法.
            if sum((tmplittle[1:-1,1:-1]==i[1:-1,1:-1]).flatten())>=0.97*duoshao:
                savign_list.append([0,1,2,3,100][dex])
                break
print("savign_list_as_1d",savign_list)
